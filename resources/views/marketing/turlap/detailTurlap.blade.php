@extends('layouts.app', ['pageTitle' => 'Data Turlap ' . $sumberMarketing->nama_sumber_marketing])
...
@section('button-action')
    <a href="{{ route('turlap.index') }}" class="btn btn-secondary btn-sm pull-right ml-2">Kembali</a>
    <a href="javascript:void(0)" data-toggle="modal" data-target="#turlapModal" class="btn btn-primary btn-sm pull-right">Input
        Turlap</a>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12">
            <div class="x_panel">
                <div class="x_title">
                    {{-- <h2>Data Turlap</h2> --}}
                    <a href="{{ route('turlap.unduhTemplateImport', $sumberMarketing->id) }}"
                    ...
                    <a href="javascript:void(0)" data-toggle="modal" data-target="#importModal"
                        class="btn btn-danger btn-sm"><i class="fa fa-upload"></i> Import Turlap</a>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="javascript:void(0)"
                                                data-url="{{ route('turlap.destroy', $marketing->id) }}"
                                                onclick="deleteData(this)"
                                                class="btn btn-danger btn-sm btn-delete">Hapus</a>
                                            <a href="{{ route('turlap.followUp', $marketing->id) }}"
    ...
    <!-- modal tambah turlap -->
    <div class="modal fade" id="turlapModal">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="turlapModalLabel">Tambah Turlap</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('turlap.store') }}" method="POST" id="turlapForm">
                        <div id="mtd"></div>
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="label" value="Turlap">
                                </div>
                                <div class="form-group">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#turlap-table').on('click', '.btn-edit', function() {
                var id = $(this).data('id');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#rating').val(response.rating_raw).trigger('change');
                        ...
                        $('#turlapModal').modal('show');

                        $('#mtd').html('<input type="hidden" name="_method" value="PUT">');
                        $('#turlapForm').attr('action', "{{ route('turlap.update', ':id') }}"
                            .replace(':id', id));
                    }
                });
            });
            ...
            $('#turlapModal').on('hidden.bs.modal', function() {
                $('#turlapForm')[0].reset();
                $('#turlapForm').attr('action', "{{ route('turlap.store') }}");
                $('#mtd').html('');
            });
        });
    </script>
