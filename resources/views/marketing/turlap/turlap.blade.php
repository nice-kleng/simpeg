@extends('layouts.app', ['pageTitle' => 'Data Area Turlap'])

@section('button-action')
    <a href="javascript:void(0)" data-toggle="modal" data-target="#sumberMarketingModal"
        class="btn btn-primary btn-sm pull-right">Input Area</a>
@endsection
                            ...
                            <tbody>
                                @foreach ($sumberMarketings as $sumberMarketing)
                                    <tr>
                                        ...
                                        <td><a href="{{ route('turlap.detailTurlapByArea', $sumberMarketing->id) }}"><span
                                                    class="badge badge-{{ ['primary', 'success', 'info', 'warning', 'danger'][$loop->index % 5] }}">{{ $sumberMarketing->nama_sumber_marketing }}</span></a>
                                        </td>
                                        <td>{{ $sumberMarketing->marketings_count }}</td>
                                        <td>
                                            <a href="javascript:void(0)" data-id="{{ $sumberMarketing->id }}"
                                                class="btn btn-primary btn-sm btn-edit"><i class="fa fa-edit"></i></a>
                                            <a href="javascript:void(0)"
                                                data-url="{{ route('sumberMarketing.destroy', $sumberMarketing->id) }}"
                                                onclick="deleteData(this)" class="btn btn-danger btn-sm"><i
                ...
                <div class="modal-body">
                    <form action="{{ route('sumberMarketing.store') }}" method="POST" id="sumberMarketingForm">
                        ...
                        <div class="form-group">
                            <label for="nama_sumber_marketing">Area</label>
                            <input type="text" name="nama_sumber_marketing" id="nama_sumber_marketing"
                                class="form-control">
        ...
        $('#turlap-table').on('click', '.btn-edit', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#sumberMarketingForm').append(
                        '<input type="hidden" name="_method" value="PUT">');
                    $('#sumberMarketingForm').attr('action',
                        "{{ route('sumberMarketing.update', ':id') }}".replace(':id', id));
                }
            });
        });
