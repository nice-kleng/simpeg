@extends('layouts.app', ['pageTitle' => 'Follow Up Turlap'])

@section('button-action')
<a href="{{ route('turlap.detailTurlapByArea', $turlap->sumber_marketing_id) }}" class="btn btn-secondary btn-sm pull-right ml-2">Kembali</a>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover text-nowrap" width="100%">
                        ...
                        <tr>
                            ...
                            <td>{{ $turlap->status_prospek }}</td>
                        </tr>
                        <tr>
                            <th>Tanggal : </th>
                            <td>{{ \Carbon\Carbon::parse($turlap->tanggal)->locale('id')->isoFormat('D MMMM YYYY') }}</td>
                        </tr>
                        <tr>
                            <th>Status Follow Up : </th>
                            <td>{{ $turlap->followUp->last()->status ?? 'Not Started' }}</td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="x_content">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover text-nowrap" id="followUp-table" width="100%">
                        ...
                        <tbody>
                            @foreach ($turlap->followUp as $followUp)
                            <tr>
...
<div class="modal fade" id="followUpModal" tabindex="-1" role="dialog" aria-labelledby="followUpModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('turlap.followUpStore', $turlap->id) }}" method="POST" id="followUpForm">
                @csrf
                @method('POST')
...
<script>
    $(document).ready(function() {
        ...
        $('#followUp-table').on('click', '.btn-edit', function () {
            ...
            $.ajax({
                type: "get",
                url: "{{ route('turlap.followUpEdit', $turlap->id) }}",
                data: {
                    id: id
                },
                dataType: "json",
                success: function (response) {                    
                    ...
                    $('#followUpForm').attr('action', "{{ route('turlap.followUpUpdate', ':id') }}".replace(':id', id));
                }
            });
        });
        ...
        $('#followUpModal').on('hidden.bs.modal', function() {
            ...
            $('#followUpForm').attr('action', "{{ route('turlap.followUpStore', $turlap->id) }}");
